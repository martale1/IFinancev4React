...
class AlligatorChartManager:
    ...
    def _add_safe_bar(self, ax, x, y, width=0.8, color='blue', **kwargs):
        """Draws vertical bars using ax.vlines to prevent C-level segfaults and run extremely fast."""
        import numpy as np
        x_arr = np.array(x)
        y_arr = np.array(y)
        ...
        collection = ax.vlines(x_arr, 0, y_arr, colors=c, linewidths=lw, label=lbl, **kw)
        try:
    ...
    def _optimize_candlestick_density(self, df, max_candles=200):
        if len(df) <= max_candles:
            return df
        print(f"[Warning] Troppi dati ({len(df)} candlestick). Ottimizzazione per migliore visualizzazione...")
        ...
        print(f"[Info] Dati ottimizzati: {len(optimized_df)} candlestick visualizzate")
        return optimized_df
    ...
    def _add_background_colors_positions(self, ax, df_pos):
        ...
        for i in range(len(df_pos)):
            ...
            if color != current_color:
                if current_color is not None and start_idx is not None:
                    segments.append((start_idx, i - 1, current_color))
                current_color = color
                start_idx = i if color is not None else None

        if current_color is not None and start_idx is not None:
            segments.append((start_idx, len(df_pos) - 1, current_color))
    ...
    def _add_candlesticks(self, ax, df, set_xticks: bool = False):
        ...
        # Disegna le wicks usando vlines (vectorized)
        if np.any(green_mask):
            ax.vlines(positions[green_mask], l[green_mask], h[green_mask], colors='darkgreen', linewidths=1.2, alpha=0.9, zorder=4)
        if np.any(red_mask):
            ax.vlines(positions[red_mask], l[red_mask], h[red_mask], colors='darkred', linewidths=1.2, alpha=0.9, zorder=4)

        # Disegna i bodies usando PatchCollection (vectorized)
        patches = []
        facecolors = []
        edgecolors = []
        ...
        for i in range(len(df)):
            ...
            if ci >= oi:
                facecolors.append('lightgreen')
                edgecolors.append('darkgreen')
            else:
                facecolors.append('lightcoral')
                edgecolors.append('darkred')

            if body_h > 0:
                rect = Rectangle((pos - candle_width / 2, body_y), candle_width, body_h)
                patches.append(rect)
            else:
                rect = Rectangle((pos - candle_width / 2, ci - 0.001), candle_width, 0.002)
                patches.append(rect)

        if patches:
            pc = PatchCollection(patches, facecolors=facecolors, edgecolors=edgecolors, linewidths=0.8, alpha=0.85, zorder=4)
    ...
    def _add_signal6_state_labels_positions(self, ax, df_pos, min_gap: int = 10):
        ...
        Le etichette complete coprivano candele e indicatori, soprattutto per gli
        stati Downtrend_revS3Sig. Il colore e la forma del marker mantengono
        l'informazione essenziale senza aggiungere riquadri o frecce.
        """
        import numpy as np
        ...
        for i in change_idx:
            ...
            # Marker rialzisti sotto la candela e ribassisti sopra, così non
            # nascondono né il corpo né le linee degli indicatori.
            if where == 'top':
    ...
    def plot_ta_dashboard(
        ...
        """
        NUOVO ORDINE PANNELLI:

          1) Price + Alligator (+SAR/MAs) con background Signal6
          2) Volume (+ MA opzionale)                         [se abilitato]
          3) ST O C H   / RSI / Williams%R (OSCILLATORI)     [se abilitati]   <-- QUI VA LO STOCASTICO
          4) MACD / Signal / Histogram
          5) MCS (+Confidence)                               [se abilitato]
          6) Technical Score                                 [se abilitato]
          7) ADX + DI (+cross, soglia 25)                    [se abilitato]   <-- ADX IN FONDO
          8) ATR% (pannello volatilità percentuale)          [se abilitato]
        """
        import matplotlib.pyplot as plt
        import numpy as np
        ...
        if show_volume:
            n_panels += 1
        if have_osc_panel:
            n_panels += 1        # nuovo pannello STOCH/RSI/WILLR
        n_panels += 1            # MACD
        if show_mcs:
            n_panels += 1
        if show_tech_score:
            n_panels += 1
        if show_adx:
            n_panels += 1        # ADX in fondo (prima di ATR%)
        if show_atr_pct_panel:
            n_panels += 1

        # Default height ratios nel nuovo ordine
        default_ratios = [4.5]  # Price
        if show_volume:
            default_ratios.append(1.8)   # Volume
        if have_osc_panel:
            default_ratios.append(2.4)   # Stoch/RSI/WILLR
        default_ratios.append(2.8)       # MACD
        if show_mcs:
            default_ratios.append(2.8)   # MCS
        if show_tech_score:
            default_ratios.append(2.2)   # TECH_SCORE
        if show_adx:
            default_ratios.append(2.4)   # ADX
        if show_atr_pct_panel:
            default_ratios.append(1.6)   # ATR %

        # Se passi height_ratios, li adatto semplicemente a n_panels
        ...
        ax_vol = None
        if show_volume:
            ax_vol = axes[idx_ax]; idx_ax += 1

        # === Stoch/RSI/WILLR (nuovo pannello oscillatori) ===
        ax_stoch = None
        if have_osc_panel:
            ax_stoch = axes[idx_ax]; idx_ax += 1
        ...
        ax_mcs = None
        if show_mcs:
            ax_mcs = axes[idx_ax]; idx_ax += 1

        # === TECH SCORE ===
        ax_ts = None
        if show_tech_score:
            ax_ts = axes[idx_ax]; idx_ax += 1

        # === ADX (ora in basso, prima di ATR%) ===
        ax_adx = None
        if show_adx:
            ax_adx = axes[idx_ax]; idx_ax += 1
        ...
        # ATR Bands - DISABLED per user request
        ...
        #     ax_price.fill_between(df_pos.index, df_pos['ATR_Lower'], df_pos['ATR_Upper'],
        #                           alpha=0.08, zorder=1)
        ...
        # ===== 3) ST O C H  / RSI / WILLIAMS%R (nuovo pannello) =====
        if have_osc_panel and ax_stoch is not None:
            have_any = False
            ax_willr = None
            if show_rsi and 'RSI' in df_pos.columns:
                have_any = True
                ax_stoch.plot(df_pos.index, df_pos['RSI'], linewidth=1.6,
                              color='#F4B000', label='RSI')
                self._add_safe_axhline(ax_stoch, 70, linewidth=4, linestyle=':', alpha=0.7)
                self._add_safe_axhline(ax_stoch, 50, linewidth=2, linestyle=':', alpha=0.7)
                self._add_safe_axhline(ax_stoch, 30, linewidth=4, linestyle=':', alpha=0.7)

            if show_stoch and all(c in df_pos.columns for c in ['Stoch_K', 'Stoch_D']):
                have_any = True
                ax_stoch.plot(df_pos.index, df_pos['Stoch_K'], linewidth=1.3,
                              color='blue', linestyle='-', label='Stoch K')
                ax_stoch.plot(df_pos.index, df_pos['Stoch_D'], linewidth=1.3,
                              color='red', linestyle='-', label='Stoch D')
                self._add_safe_axhline(ax_stoch, 80, linewidth=0.8, linestyle=':', alpha=0.7)
                self._add_safe_axhline(ax_stoch, 20, linewidth=0.8, linestyle=':', alpha=0.7)

            if show_willr and 'Williams_R' in df_pos.columns:
                have_any = True
                if willr_shifted:
                    # Plot shifted Williams %R (0..100) on the same main axis
                    ax_stoch.plot(df_pos.index, df_pos['Williams_R'] + 100, linewidth=1.2,
                                  color='#4682B4', label='Williams %R (shifted)')
                else:
                    # Plot native Williams %R (-100..0) on secondary y-axis
                    ax_willr = ax_stoch.twinx()
                    ax_willr.plot(df_pos.index, df_pos['Williams_R'], linewidth=1.2,
                                  color='#38bdf8', label='Williams %R')
            ...
            if have_any:
                ...
                ax_stoch.text(0.02, 0.7, "Oscillatori non disponibili", transform=ax_stoch.transAxes)
        ...
        if all(c in df_pos.columns for c in ['MACD', 'MACD_Signal', 'MACD_Hist']):
            ...
            try:
                self._add_safe_bar(ax_macd, idx_x, hist, width=0.8, alpha=0.7, color='green',
                                   label='MACD Histogram', zorder=1)
            except Exception:
        ...
        if show_mcs and ax_mcs is not None:
            ...
            if all(c in df_pos.columns for c in ['MCS', 'MCS_Smoothed']):
                ...
                self._add_safe_axhline(ax_mcs, TH_UP, color='#157347', linewidth=2.2,
                                       linestyle=(0, (6, 3)), alpha=0.95, zorder=1)
                ...
                if cross_up.any():
                    ax_mcs.scatter(idx_x[cross_up], ms[cross_up], marker='^', s=110,
                                   color='green', edgecolors='black', linewidth=1.1,
                                   zorder=6, label='Cross > 0 (BUY)')
                if cross_dn.any():
                    ax_mcs.scatter(idx_x[cross_dn], ms[cross_dn], marker='v', s=110,
                                   color='red', edgecolors='black', linewidth=1.1,
                                   zorder=6, label='Cross < 0 (SELL)')

                alert_up = (ms.shift(1) <= TH_UP) & (ms > TH_UP)
                alert_dn = (ms.shift(1) >= TH_DN) & (ms < TH_DN)
                if alert_up.any():
                    ax_mcs.scatter(idx_x[alert_up], ms[alert_up], marker='o', s=70,
                                   color='orange', edgecolors='black', linewidth=1.0,
                                   zorder=6, label='> +30 (Momentum forte)')
                if alert_dn.any():
                    ax_mcs.scatter(idx_x[alert_dn], ms[alert_dn], marker='o', s=70,
                                   color='purple', edgecolors='black', linewidth=1.0,
                                   zorder=6, label='< -30 (Momentum forte)')
        ...
        # ---- X axis & grid ----
        # asse X sull’ULTIMO pannello (in basso)
        bottom_ax = axes[-1]
    ...
    def plot_ta_dashboardv0(
        ...
        if 'Signal6' in df_pos.columns:
            self._add_signal_markers_positions(ax_price, df_pos)
            self._add_signal6_state_labels_positions(ax_price, df_pos, min_gap=10)
        # --- ATR Bands (Upper/Lower) - DISABLED per user request ---
        # if show_atr_band and {'ATR_Upper', 'ATR_Lower'}.issubset(df_pos.columns):
        #     ax_price.plot(df_pos.index, df_pos['ATR_Upper'], linestyle='--', linewidth=1.0, alpha=0.8,
        #                   label='ATR Upper', zorder=2)
        #     ax_price.plot(df_pos.index, df_pos['ATR_Lower'], linestyle='--', linewidth=1.0, alpha=0.8,
        ...
        if show_mcs and (ax_mcs is not None):
            if all(c in df_pos.columns for c in ['MCS', 'MCS_Smoothed']):
                ...
                # show_mcs=True ma colonne mancanti: mostra messaggio sul pannello esistente
                ax_mcs.text(0.02, 0.7, "MCS non disponibile", transform=ax_mcs.transAxes)
