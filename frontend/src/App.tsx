  ...
  return (
    <main className="app">
      ...
      {tab === "📊 Highlights" && watchlistQuery.data ? <section className="highlights-panel">
        ...
        <div className="highlights-list"><h3>Titoli in evidenza</h3>{[...watchlistQuery.data!.items].sort((a,b) => Number(b.ADX ?? 0) - Number(a.ADX ?? 0)).slice(0, 8).map((row) => { const adx = Number(row.ADX); const plus = Number(row.PLUS_DI); const minus = Number(row.MINUS_DI); const direction = adx >= 25 && plus > minus ? "up" : adx >= 25 && plus < minus ? "down" : "weak"; return <button className={`highlight-row ${direction === "up" ? "trend-up" : direction === "down" ? "trend-down" : "bounce"}`} key={String(row.Ticker)} onClick={() => openChart(row)}><strong>{String(row.Ticker)}</strong><span>{String(row.Name ?? "-")}</span><b>ADX {adx.toFixed(1)} · DI+ {plus.toFixed(1)} · DI− {minus.toFixed(1)}</b><em>{direction === "up" ? "Trend rialzista" : direction === "down" ? "Trend ribassista" : "Movimento debole"}</em></button>; })}</div>
