  ...
  return (
    <main className="app">
      ...
      {tab === "📊 Highlights" && watchlistQuery.data ? <section className="highlights-panel">
        ...
        <div className="highlights-list"><h3>Titoli in evidenza</h3>{[...watchlistQuery.data!.items].sort((a,b) => Number(b.ADX ?? 0) - Number(a.ADX ?? 0)).slice(0, 8).map((row) => { const trend = Number(row.PLUS_DI) > Number(row.MINUS_DI) && Number(row.ADX) >= 25; return <button className={`highlight-row ${trend ? "trend-up" : "bounce"}`} key={String(row.Ticker)} onClick={() => openChart(row)}><strong>{String(row.Ticker)}</strong><span>{String(row.Name ?? "-")}</span><b>ADX {Number(row.ADX ?? 0).toFixed(1)} · DI+ {Number(row.PLUS_DI ?? 0).toFixed(1)} · DI− {Number(row.MINUS_DI ?? 0).toFixed(1)}</b><em>{trend ? "Trend rialzista" : "Rimbalzo"}</em></button>; })}</div>
